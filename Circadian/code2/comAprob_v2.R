...
dfmax=read.table("../final_Binary_and_Count.txt")
NPheno=length(unique(dfmax$V1)) # mmax , uniqM
rm(dfmax)

df0=read.table("results_circadian.dat",header = T)
df0=df0[,c(-2,-3,-5)]
...
for (indi in allindis) {
    ...
    pdf(paste(indi,"_CI.pdf",sep=""))
     par(mar = c(5, 6, 4, 2) + 0.1)  # Adjusting margin
     par(cex.axis = 1.5, cex.lab = 1.5)
    
     plot(x=df1$ConditionalComplexity,y=df1$Freq,
          main = paste("complexity",df1$Complexity[1]),
          cex=2.5,pch=16,col="blue",ylim = c(min(df1$Freq),0),
          xlab = expression(paste(~tilde(K), "(y|x)")),
          ylab = expression(paste("log"[10], " P(x" %->% "y)")))
     #for (i in 1:n_boot) {
    #   aaa=bf[[i]]$coefficients[2] 
    #   b=bf[[i]]$coefficients[1]
    #   abline(b, aaa, col=rgb(0, 0, 1, alpha=0.1), lty=3, lwd=0.01)
    # }
     #points(x=df1$ConditionalComplexity,y=df1$Freq,
    #      cex=2.5,pch=16,col=color1)
     points(dfmx,col="black",pch=16,cex=1.5)
     lines(ndf1$x, fit1$family$linkinv(pred1), col="black",lwd=3)
     for (j in 1:2) lines(ndf1$x, fit1$family$linkinv(bpredci[, j]), col="black", lty=2,lwd=3)
     abline(model1,col="red",lwd=2)
    
     
     
      #legend("topright",legend = c("Fitted model","Fitted 95% CI",
      #            #expression(paste("Bound model",~tilde(K), "(y|x)"))), 
      #            "Bound model","all fitted bootstrap"),
      # lwd = c(2.5, 2.5,2.5,1),lty=c(1,2,1,3),       # Point symbols like the plot
      # col = c("black","black", "red","blue")) # Colors for the symbols
      
      
      legend("topright",legend = c("Fitted model","Fitted 95% CI","Bound model"),
             lwd = c(2.5, 2.5,2.5),lty=c(1,2,1),       # Point symbols like the plot
             col = c("black","black", "red")) # Colors for the symbols
      ...
      #text(16,-0.25,paste("Level 1: corr",round(corr$estimate,4),"pval",round(corr$p.value,4)))
      #text(16,-0.50,paste("Level 2: lm pval",round(model_p_value,4)))
      #text(16,-0.75,paste("Level 3: bootstrap",a))
      
      # Adding text stacked in the bottom left corner
      text(x = par("usr")[1] + 0.5, y = par("usr")[3] + 0.75, 
           labels = paste("Level 1: corr", round(corr$estimate, 4), "pval", round(corr$p.value, 4)), 
           pos = 4, adj = c(0, 0))
      text(x = par("usr")[1] + 0.5, y = par("usr")[3] + 0.5, 
           labels = paste("Level 2: lm pval", round(model_p_value, 4)), 
           pos = 4, adj = c(0, 0))
      text(x = par("usr")[1] + 0.5, y = par("usr")[3] + 0.25, 
           labels = paste("Level 3: bootstrap", a), 
           pos = 4, adj = c(0, 0))
...
boxplot(save.stas$spearman,
        ...
        xlab="all phenotypes",ylab="correlations")
abline(0,0)
dev.off()
...
boxplot(save.stas$pvallinear,
        main="Level 2 \n linear regression signicant",
        xlab="all phenotypes",ylab="pvalues")
abline(0.05,0)
dev.off()
