...
NPheno=520 #taken from global exploration


df0=read.table("allbmbs.dat",header = T)

df0$ConditionalComplexity = pmin(abs(df0$PaCom-df0$OPC),df0$OPC)

colnames(df0) = c("File","Mutant","Parent","ConditionalComplexity")
...
allindis=unique(df0$Parent)
df=data.frame()
for (indi in allindis) {
  df.tmp=df0[which(df0$Parent==indi),]
  df2 <- df.tmp %>%
    group_by_at("Mutant") %>%   
    mutate(n = n()) %>% 
    distinct()
...
for (indi in allindis) {
    ...
    minkc=min(df1$ConditionalComplexity)
    maxkc=max(df1$ConditionalComplexity)
    interv = maxkc / 10
    maxis=data.frame()
    for (iji in seq(minkc,maxkc-interv,interv) ) {
     df.tmp=df1[which(df1$ConditionalComplexity>iji & df1$ConditionalComplexity<=iji+interv),]
     id=which(df.tmp$Freq==max(df.tmp$Freq))[1]
     if(length(id)>0){
     m.tmp=data.frame(Max_ckc=df.tmp$ConditionalComplexity[id],Max_freq=df.tmp$Freq[id])  
     maxis=rbind(maxis,m.tmp)
     }
    }
    maxis20=maxis[which(maxis$Max_freq>-5),]
    colnames(maxis20)=c("ConditionalComplexity","Max_prob")
...
for (iji in 1:nrow(save.stas)) {
    if(save.stas$pvallinear[iji]>0.05 | save.stas$pearson[iji]<0.5 ){
...
pdf("stats_teethOPC_pear_r2.pdf")
par(mar = c(5, 6, 4, 2) + 0.1)  # Adjusting margin
par(cex.axis = cex_axis, cex.lab = cex_lab, cex.main=cex_main)
