...
dfmax=read.table("../HP_output_data/HPn25_unique_str_count_prob.txt")
NPheno=length(unique(dfmax$V1)) # mmax , uniqM
rm(dfmax)

df=read.table("results_hp.dat",header = T)
#colnames(df) = c("parent","mutant","")
df1 <- df[!grepl("ParentPhenotyope", df[, 1]), ]
#df1 <- as.data.frame(lapply(df0, as.numeric))

allindis=unique(df1$ParentPhenotyope)
save.stas=data.frame()
for (indi in allindis) {

    df1=df[which(df$Parent==indi),]
    df1=df1[order(as.numeric(df1$Conditional_Complexity)),]
    df1$Freq=log10(as.numeric(df1$proportion))
    
    NPheno=length(unique(df1$Mutant))
    
    minC= min(as.numeric(df1$Conditional_Complexity)) ; maxC= max(as.numeric(df1$Conditional_Complexity))
    upper_bound=df1
    upper_bound$Z2=log2(NPheno)*((as.numeric(df1$Conditional_Complexity)-minC)/(maxC-minC))
    
    maxis <- df1 %>%
      group_by(Conditional_Complexity) %>%
    ...
    # Extract x and y
    x <- as.numeric(maxis20$Conditional_Complexity)
    y <- as.numeric(maxis20$Max_prob)
    ...
    model <- lm(Max_prob ~ as.numeric(Conditional_Complexity), data = maxis20)
    model1 <- lm(Max_prob2 ~ Conditional_Complexity, data = upper_bound)
    color1=rgb(0, 0, 0, 0.75)
    
    pearson.r2 = (cor(as.numeric(maxis20$Conditional_Complexity),maxis20$Max_prob))**2
    ######################    test bootstrap
    x1=as.numeric(maxis20$Conditional_Complexity) ; y1=as.numeric(maxis20$Max_prob)
    x2=as.numeric(upper_bound$Conditional_Complexity) ; y2=as.numeric(upper_bound$Max_prob2)
    ...
    pdf(paste(indi,"_c_shade_CI.pdf",sep=""))
     par(mar = c(5, 6, 4, 2) + 0.1)  # Adjusting margin
     par(cex.axis = cex_axis, cex.lab = cex_lab , cex_main=cex_lab)
    
     plot(x = df1$Conditional_Complexity, y = df1$Freq,
          #main = paste("complexity", indi),
          xaxt = 'n', yaxt = 'n', xlab = "", ylab = "",
          cex = cex2, pch = 16, col = "blue", ylim = c(min(df1$Freq), 0))
         # xlab = expression(paste(~tilde(K), "(y|x)")),
         # ylab = expression(paste("log"[10], " P(x" %->% "y)")))
     
     axis(1,cex=cex2,line=0)
     axis(2,cex=cex2,line=0)
     mtext(expression(paste(~tilde(K), "(y|x)")), side = 1, line = 4,cex=cex2)  
     mtext(expression(paste("log"[10], " P(x" %->% "y)")), side = 2, line = 3.5,cex=cex2)
     ...
     legend("topright",legend = c("Fitted model","Fitted 95% CI","Bound model"),
      ...
      corr=cor.test(df1$Freq,as.numeric(df1$Conditional_Complexity),method="spearman")
      ...
      x <- as.numeric(c(maxis20$Conditional_Complexity, upper_bound$Conditional_Complexity))
    ...
    pdf(paste(indi,"_c_shade_CI_stats.pdf",sep=""))
    par(mar = c(5, 6, 4, 2) + 0.1)  # Adjusting margin
    par(cex.axis = cex_axis, cex.lab = cex_lab , cex_main=cex_lab)
...
par(mar = c(5, 6, 4.5, 2) + 0.1)  # Adjusting margin
...
pvallinear = na.omit(save.stas$pvallinear)
for (iji in 1:length(pvallinear)) {
  if(pvallinear[iji]>0.05){
