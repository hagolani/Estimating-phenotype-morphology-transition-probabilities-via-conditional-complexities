...
dfmax=read.table("../in_out_count.txt")
NPheno=length(unique(dfmax$V2)) # mmax , uniqM
rm(dfmax)

df=read.table("results_multmatrix.dat",header = T)
...
for (indi in allindis) {
    ...
    pdf(paste(indi,"_",df1$PaComplexity[1],"_shade_CI.pdf",sep=""))
    ...
    pdf(paste(indi,"_",df1$PaComplexity[1],"_shade_CI_stats.pdf",sep=""))
    par(mar = c(5, 6, 4, 2) + 0.1)  # Adjusting margin
    par(cex.axis = cex_axis, cex.lab = cex_lab , cex_main=cex_lab)
    ...
    # Get the plot limits (x1, x2, y1, y2)
    plot_limits <- par("usr")
...
pdf("stats_circadian_pear_r2.pdf")
par(mar = c(5, 6, 4, 2) + 0.1)  # Adjusting margin
par(cex.axis = cex_axis, cex.lab = cex_lab, cex.main=cex_main)
